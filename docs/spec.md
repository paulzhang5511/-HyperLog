...
| # | 指标 | 目标 | 实测 | 结论 |
| --- | --- | --- | --- | --- |
| P1 | 打开 1 GB（≈1000 万行） | < 2.0 s | **0.98–1.04 s** | ✅ 余量充足（并行 `memchr` 扫描替代单线程逐字节循环） |
| P3 | 索引常驻内存 | ≤ 8 B/行 | 8.00 B/行（80 MB / 1000 万行） | ✅ 恰好达标（`Vec<usize>` 8 字节/行） |
| P7 | 正则检索吞吐 | ≥ 200 MB/s | **1425.3 MB/s** | ✅ 余量充足（M7 块级并行扫描） |
| P8 | 纯文本（字面量）检索吞吐 | ≥ 800 MB/s | **2073.6 MB/s** | ✅ 余量充足（M7 块级并行扫描） |
...
> （各块字节区间互不重叠、命中行无重复，跨块无需去重；取消检查在波次之间 < 500 ms），P7 升至
> 1425 MB/s、P8 升至 2074 MB/s。未自动化项（需 GUI 手工观测，代码已就位）：P2/P4/P5/P6/P9/P10/P11/P12。
