...
| 2026-09-06 | **顶栏改版（VS Code 风格双行分组）**：原工具栏所有控件（打开/打开目录/最近/查找输入/模式/Aa/查找/查找全部/命中/仅命中/导出/带前缀/行号/折行/右拆/下拆/关闭面板/性能/主题）平铺在单行 `horizontal_wrapped`，窗口稍窄就互相挤压；视觉上也过于「列表化」，缺乏结构感。改版为：① **双行布局**——第一行主操作栏（左段品牌+侧栏开关+文件组、中段居中放大查找栏、右段性能+主题），第二行上下文/视图栏（行号跳转+折行+拆分面板组+命中结果组），通过 `Panel::top` 内 `allocate_ui_with_layout` 强制固定行高（22px）让两行不互相挤压。② **常用操作图标化**——打开/打开目录/最近 三个高频按钮改用 📂/📁/🕘 图标（带 hover 提示，文字保留为可访问性提示）；其余低频操作（拆分/导出/折行）保留文字。③ **查找栏居中放大**——第一行内用「左段 + 12px 留白 + 380px 查找栏 + 吃光剩余宽度 + 右段 right_to_left」的三段写法实现视觉居中（窗口 ≥580px 时固定 380，<580 时缩到 160）。④ **顶栏底部分隔线**——用 `painter.hline` 画 1px `border` 色横线，让顶栏与正文区有明确分隔（编辑器观感而非 egui 默认零分隔）。所有语义不变（`pending_*` 标志、`save_prefs` 时机、`apply_wrap_to_active_pane` 等），仅布局重排。门禁 fmt/clippy(-D warnings)/87 单测/release 构建均绿 | Agent |
| 2026-09-06 | **发布三平台二进制（v0.0.5 准备）**：① `release.yml` matrix 由「单 macOS + Windows」扩为**三目标**——macOS Apple Silicon (`macos-latest`，arm64)、macOS Intel (`macos-15-intel`，x86_64)、Windows x64 (`windows-latest`)；`dtolnay/rust-toolchain` 加 `targets:` 安装对应 Rust target，macOS 走 `package_macos.sh <target>`、Windows 用 `cargo build --release --target` 后 `Compress-Archive`。② `package_macos.sh` 支持架构参数（短名 `aarch64`/`x86_64` 或完整 rust target），按 host 与 target 是否一致决定原生/交叉编译，zip 命名区分 `HyperLog-macos-arm.zip` / `HyperLog-macos-intel.zip`。③ **关键坑**：`macos-13`（原 Intel runner）已于 2025-12-04 由 GitHub 退役，Intel Mac 构建必须改用 `macos-15-intel`（最后一个 x86_64 macOS runner，可用至 ~2027-08）；沿用 `macos-13` 会导致 job 无限排队到 24h 自动取消。④ 版本号 0.0.4 → 0.0.5（`Cargo.toml`/`Cargo.lock`/`docs/prd.md`） | Agent |
| 2026-09-06 | **三项打磨**：① **启动固定单面板**——拆分面板改为会话内临时状态，`LogViewerApp::new` 里 `pane_layout` 恒为 `{dir: Horizontal, count: 1, ratio: prefs.split_ratio}`，不再从偏好恢复拆分数/方向；`Prefs` 删除 `split_count`/`split_vertical` 字段（`load_from` 保留这两个键为「历史键忽略」以兼容旧配置，`save_to` 不再写），仅保留 `split_ratio`（下次拆分时的布局偏好）。② **关闭图标**——确认现状已满足：每个面板标题栏右侧在 `count > 1` 时都渲染 ✕（`render_one_pane` 对每个 pane 都执行），单面板时不显示（避免关闭唯一面板后无面板可用）。③ **修复双击选中文字看不清**——根因：egui 文本选区 `paint_text_selection` 会把选中文字顶点色改写为 `selection.stroke.color`，而 `theme.rs` 误设为 `p.accent`（蓝），蓝字叠在蓝底上暗色主题对比度仅 **1.33:1**、亮色 2.97:1，双击选词后几乎不可见。修复：`selection.stroke.color` 改用 `p.text_strong`（暗色白 8.5:1 / 亮色近黑 11.47:1）。新增回归测试 `selected_text_is_readable_on_selection_background`（88 单测）。门禁 fmt/clippy(-D warnings)/88 单测全绿 | Agent |
| 2026-09-06 | **关闭图标始终显示 + 单面板关闭语义**：用户运行后发现单面板时标题栏缺关闭图标。此前 ✕ 仅在 `pane_layout.count > 1` 时渲染（上一轮「三项打磨」误判为保持现状）。修复：`render_one_pane` 里 ✕ 按钮**始终显示**，`hover_text` 随面板数区分「关闭面板 / 关闭文件」；多面板点 ✕ 走原 `close_pane`，单面板点 ✕ 走新增 `AppState::close_current_file()`——清空全局 `fileset`、检索态（`search_results`/`search_truncated`/`search_error`/`hit_regex`）、每面板视图态（`clear_pane_view_states` 含 override）、顶层镜像（`selected_row`/`sidebar_active_file`/`dirty_files`/`max_line_width`/`in_result_mode`），状态栏提示「未打开文件」。新增 2 个单测（`close_current_file_clears_fileset_and_view_state` / `close_current_file_drops_pane_override`），90 单测全绿 | Agent |
| 2026-09-06 | **压缩面板标题条高度**：用户反馈每个面板顶部有空白高度。根因：标题条用 `ui.horizontal` 渲染，而其行高直接取 `spacing.interact_size.y`（theme.rs 里设为 22px），叠加 `button_padding.y=3` 后标题条高达 26px，12px 字号文字上下各留约 5px 空白。修复：`render_one_pane` 用 `ui.scope` 隔离收紧 spacing——`interact_size.y` 22→16、`button_padding.y` 3→0、`item_spacing.y` 4→2，标题条 26px→20.5px。关键：必须用 `scope` 在 `horizontal` **之外**收紧（`horizontal` 的行高在调用时即取 `interact_size.y`，闭包内改无效），且 `scope` 用 `new_child` 拷贝 spacing 不会泄漏到正文区 | Agent |
| 2026-09-06 | **面板标题条图标化（对齐 VS Code 编辑器组）**：用户反馈面板设计不符合预期，希望每个面板右侧有叉号并参考 VS Code 面板拆分/图标设计。此前右侧是 `small_button("共享"/"本面板")` + `small_button("✕")` 文字按钮（带边框背景，观感偏「按钮」而非「图标」）。改为：① 标题条右侧图标区统一为**扁平无边框图标**（`right_to_left`：最右关闭、往左拆分、再往左单独打开/返回共享），新增 `titlebar_icon` helper——`allocate_exact_size(18×18)` 命中区，默认仅画图标本体，hover 才铺 `row_hover` 浅色圆角背景、按下铺 `control_hover`，禁用用 `text_dim.gamma_multiply(0.45)` 淡化；② **关闭**用字符 `×`（U+00D7）、**单独打开**用全角 `＋`（U+FF0B，MiSans 确定覆盖）；③ **拆分**与**返回共享**改为 `Painter` 自绘矢量（拆分=矩形外框+中线竖线，返回共享=左箭头），避开几何符号/Arrows 区（如 U+21A9 `↩`、`▥`）在 egui 内置字体链（Ubuntu-Light/NotoEmoji/MiSans）中字形覆盖不可靠、易渲染成豆腐块的风险；④ 拆分动作固定为**向右拆分**（`SplitDir::Horizontal`，VS Code 默认 split 方向），达 `MAX_PANES` 上限时禁用；「共享/本面板」语义不变（仍写/清 `fileset_override`）。门禁 fmt/clippy(-D warnings)/90 单测全绿 | Agent |
| 2026-09-06 | **修复标题条图标不可见（绘制顺序根因）**：上一轮图标化后用户运行发现三个图标完全没出现（标题条一片空白）。根因是**绘制顺序颠倒**——egui 的 `Painter` 把 shape 按调用顺序压进同一图层，**后压的覆盖先压的**；原实现是「先 `ui.scope`+`ui.horizontal` 画文件名/图标 → 再在 `horizontal` 之后用 `ui.painter().rect_filled(title_rect,..)` 铺标题条背景」，于是背景把整个标题条内容（含文件名与全部图标）盖掉了。这也解释了更早用户反馈的「每个面板顶部有一个空白的高度」——那处空白正是被背景盖住的内容区，而非 padding。修复：改用 `ui.allocate_ui_with_layout(vec2(avail_w, TITLE_BAR_HEIGHT=20.0), left_to_right)` 先定下固定高度标题条矩形，在闭包内**先铺背景**（`ui.painter().rect_filled(ui.max_rect(),..)`）、**再画内容**（`scope` 收紧 spacing + `horizontal` 画文件名与图标）；删除原「事后补背景」的 `title_rect` 计算与 `title_top` 变量（`allocate_ui_with_layout` 已自动推进父 ui cursor，正文仍取 `available_rect_before_wrap()`）。新增常量 `TITLE_BAR_HEIGHT`。**定式：egui 中「背景 + 前景内容」必须背景先画，绝不能事后补背景**。门禁 fmt/clippy(-D warnings)/90 单测全绿，release 二进制启动探活正常（Glow，加载文件无报错） | Agent |
| 2026-09-06 | **发布 0.0.6**：版本号 `Cargo.toml`/`Cargo.lock`/`docs/prd.md` 0.0.5→0.0.6。本版本相对 0.0.5 的变更：① 面板标题条图标化（关闭 `×` / 拆分自绘图标 / 单独打开 `＋` / 返回共享自绘左箭头，扁平无边框、hover 才显背景）；② 修复标题条图标不可见（绘制顺序：背景必须画在内容之前，此前背景把标题条内容整片盖掉）；③ 启动固定单面板；④ 关闭图标始终显示 + 单面板点 ✕ 关闭当前文件；⑤ 修复双击选中文字看不清（选区前景色改用 `text_strong`）；⑥ 压缩面板标题条高度（26px→20px）。发布前用临时 `HYPER_LOG_DBG` 实测布局：标题条固定 20px、正文 688px（无「正文高度归零」回归） | Agent |
| 2026-09-06 | **内容区编辑器式交互（参考 VS Code）**：正文（日志区）补齐编辑器习惯操作。① **多行选择 + 批量复制**——`PaneState` 新增 `selection_anchor: Option<usize>`，与 `selected_row` 共同表示一段**闭区间** `[min,max]`（日志可达上亿行，存区间只需两个 `usize`，⌘A 全选也不爆内存）；新增 `selected_range()`/`is_row_selected()`/`selected_count()`/`extend_selection_to()`/`select_single()`/`select_all()`/`clear_selection()`。交互：普通点击=单行选中、Shift+点击=以原选中为锚点扩展到该行、⌘A=全选；右键菜单补「复制选中的 N 行」（多行时）/「复制此行」，⌘C 复制整段区间。② **批量复制防内存爆炸**——`MAX_COPY_ROWS=100_000`，`collect_selected_rows` 超上限只取前 N 行并 `log::warn`。③ **键盘导航**（VS Code 风）——活动面板响应 ↑↓ 移动光标行、Shift+↑↓ 扩展选区、PageUp/Down 整页翻动、Home/End 跳首/尾，移动后借 `scroll_target` 滚入可视区；⌘C/⌘A 仅活动面板响应（多面板「先到先得」问题，同 M18）。④ **选中视觉增强**——光标行（`selected_row` 主端点）加左侧 accent 竖条 + 行号加亮为 `text_strong`，整段选中区间铺 `row_active` 底色，对齐 VS Code 当前行观感。⑤ **借用修复**——⌘C 处 `fileset` 不可变借用与 `pane.select_*` 可变借用冲突，改为复制处即时重新派生 `fs` 局部借用。**踩坑**：多行选择代码在速率限制前未编译，落地时暴露此借用错误（NLL：`fileset` 在闭包外仍被引用的写法会跨过可变借用）。单测 90→95（锚点选区模型 5 个），门禁 fmt/clippy(-D warnings) 全绿，启动探活正常 | Agent |
| 2026-09-06 | **行号槽拖动连续多选（参考 VS Code）**：补齐内容区最后一个未做的编辑器操作。① **`PaneState` 新增 `drag_anchor: Option<usize>`**——记录一次拖拽手势的起点行，与 `selection_anchor` 区分（后者是跨帧稳定的选区端点）。② 行号槽 `allocate_exact_size` 的 `Sense` 由 `click` 改为 `click_and_drag`；`drag_started` 时：Shift 按住则保留既有选区为固定端扩展（`extend_selection_to` + `drag_anchor = 既有锚点`）、否则从该行起新建选区（`select_single` + `drag_anchor = 该行`）。③ 拖动中每帧记录指针**悬停**到的行（`gutter_resp.hovered()`），闭包结束后把 `selection_anchor = drag_anchor`、`selected_row = hover`，即「按住行号槽上下拖 = 连续多选」。④ **边缘自动滚屏**：指针进入可视区上下 `2*row_h` 边缘时设 `scroll_target` 让内容滚动，以便选中屏幕外行（VS Code 行为）；该块放在消费 `scroll_target` **之前**，使自动滚动当帧生效。⑤ **借用修复**：`fileset` 原是 `&pane.fileset_override` 的不可变借用，闭包内新增 `pane.select_*` 可变借用导致 E0500；改为持有 `fileset` 所有权（`FileSet` 是 `Vec<Arc<..>>` 浅克隆，开销可忽略），断开对 `pane` 的借用。⑥ `clear_selection` 一并清空 `drag_anchor`（文件重载等重置时取消进行中的拖拽）。门禁 fmt/clippy(-D warnings)/95 单测全绿，启动探活正常 | Agent |
| 2026-09-06 | **发布 0.0.7**：版本号 `Cargo.toml`/`Cargo.lock`/`docs/prd.md` 0.0.6→0.0.7。本版本相对 0.0.6 的变更（内容区编辑器式交互，参考 VS Code）：① **多行选择 + 批量复制**——`PaneState` 加 `selection_anchor`，与 `selected_row` 表示闭区间；普通点击=单行、Shift+点击/行号槽拖动=扩展、⌘A=全选；⌘C/右键复制整段区间，`MAX_COPY_ROWS=100_000` 防内存爆炸；② **键盘导航**——活动面板 ↑↓ 移动、Shift+↑↓ 扩展、PageUp/Down 翻页、Home/End 首尾，移动后滚入可视区；③ **选中视觉增强**——光标行左侧 accent 竖条 + 行号加亮 `text_strong`，区间铺 `row_active`；④ **行号槽拖动连续多选**（含边缘自动滚屏）——`drag_anchor` 跟踪起点，悬停行确定终点；⑤ 面板标题条图标化、绘制顺序修复、双击选区前景色、标题条压缩等 0.0.6 既有修复均包含。单测 90→95 | Agent |
| 2026-09-07 | **内容区第二波 VS Code 化（检索导航 + 字号缩放 + 选词联动检索）**：① **检索导航增强**——`AppState` 新增 `hit_cursor: Option<usize>`（当前命中游标）；新增 `goto_hit(dir)`：命中视图按命中索引定位、全量视图换算全局行号（`file_global_start + line_idx`），两端环绕（末尾→首个），重搜后命中变少先夹取再减防下溢；`HitDir{Next,Prev}` 枚举。快捷键：`F3`/`⇧F3`（`key_pressed`+`modifiers.shift` 判向，避开 `consume_shortcut` 的「包含」语义把 F3 当 ⇧F3）、`⌘G`/`⇧⌘G`（先判 ⇧⌘G，`Modifiers::matches` 是包含语义）、⌘↵ 仍等价查找。工具栏检索框旁显示「第 N / 共 M 处」（游标存在时）否则退回「N 命中」，加「上/下」按钮（中文单字而非 Unicode 箭头，避字体覆盖）。② **字号缩放**——`Prefs` 新增 `font_size`（默认 12.5、范围 8–28，core 层常量 `DEFAULT/MIN/MAX_FONT_SIZE`，core 禁依赖 GUI）；`⌘+`/`⌘-`/`⌘0` 缩放并落盘；`set_font_size` 夹取后清空各面板 `max_line_width`（行高/行号槽宽度由字号派生，不清会横向滚动范围停留在旧值）；`estimate_text_width`/`estimate_content_width` 加 `font_size` 参数按 `font_size/LOG_FONT_SIZE` 等比缩放字宽估算。③ **选词联动检索**——`PaneState` 新增 `selected_word: String`（放面板而非 AppState 因取词在行渲染闭包只拿得到 `&mut PaneState`）；双击时按点击位置反查字符索引取词（`layout_no_wrap`+`cursor_from_pos` 拿 `CCursor`，其 `.index` 是 `CharIndex` 需 `.into()` 转 usize），存进面板；`⌘F` 时若有词则自动填入检索框并聚焦全选；`clear_pane_view_states` 一并 `selected_word.clear()`（旧文档词无意义）。④ **取词边界**——`word_at`/`is_word_char`：排除空白与 `,;()[]{}"'<>|`，其余（含 `.` `:` `-` `=`）算词内，故 `com.example.Foo:42`、`code=42` 整体取到。⑤ **修复测试 flaky**——`tmp_log` 并行 `#[test]` 共用固定名 `a.log`，一个线程 `write` 截断重写时另一线程 `LogFileIndex::open` 撞 0 字节空窗 → `Empty` → `loaded==0` 提前 return；改为 `AtomicUsize` 递增序号唯一化文件名。单测 95→103（命中环绕/字号夹取/取词边界 8 个），门禁 fmt/clippy(-D warnings)/103 单测全绿，启动探活正常 | Agent |
| 2026-09-07 | **发布 0.0.8**：版本号 `Cargo.toml`/`Cargo.lock`/`docs/prd.md` 0.0.7→0.0.8。本版本相对 0.0.7 的变更（内容区第二波 VS Code 化）：① **检索导航**——`hit_cursor` + `goto_hit`，F3/⇧F3、⌘G/⇧⌘G 命中间前后跳（环绕、重搜夹取防下溢），工具栏「第 N / 共 M 处」+ 上/下按钮；② **字号缩放**——`Prefs.font_size`（默认 12.5 / 8–28），⌘+/⌘-/⌘0 落盘，字号变化清 `max_line_width` 且字宽估算等比缩放；③ **选词联动检索**——`PaneState.selected_word`，双击取词（`layout_no_wrap`+`cursor_from_pos`，`CCursor.index` 是 `CharIndex` 需 `.into()`），⌘F 自动带入；取词边界含 `.` `:` `-` `=`。④ 修复测试 flaky（`tmp_log` 并行固定名竞争 → `AtomicUsize` 唯一化）。单测 95→103 | Agent |
| 2026-09-07 | **右键菜单增强（参考 VS Code）**：补齐内容区右键菜单的最后几项。① **复制为带行号**——新增 `collect_selected_rows_with_numbers`，按「`行号: 内容`」格式拼接（行号取 `row_content` 的 `gutter_text`：全量视图 1 起全局行号、命中视图 `<文件序>.<行号>`），同样受 `MAX_COPY_ROWS` 上限约束；多行选中复制整段带行号、单行复制当前行。② **全选**——右键调 `pane.select_all(total-1)`（上亿行也只两个 `usize`）。③ **查找选中词**——优先用双击取到的 `pane.selected_word`，否则回退当前行 `trim`；因 `log_view::show` 的 `state` 是 `&AppState` 不可变借用，无法直接写 `search_pattern`，故 `show` 改为返回 `Option<String>`（要查的词）上抛，由 `render_one_pane`（`&mut AppState`）落实为 `search_pattern` + `focus_search`（只带入不自动跑检索，让用户改词后回车）。④ 菜单用 `ui.separator()` 分组：复制组 / 全选 / 查找。单测 103→104（带行号格式），门禁 fmt/clippy(-D warnings)/104 单测全绿，启动探活正常 | Agent |
| 2026-09-07 | **发布 0.0.9**：版本号 `Cargo.toml`/`Cargo.lock`/`docs/prd.md` 0.0.8→0.0.9。本版本相对 0.0.8 的变更（右键菜单增强，参考 VS Code）：① **复制为带行号**——`collect_selected_rows_with_numbers`，`行号: 内容` 格式（全量 1 起全局行号 / 命中视图 `<文件序>.<行号>`），受 `MAX_COPY_ROWS` 约束；② **全选**——右键 `select_all(total-1)`；③ **查找选中词**——优先双击取词 `selected_word` 否则回退当前行，`log_view::show` 改返回 `Option<String>` 上抛，由 `render_one_pane` 落实为 `search_pattern`+`focus_search`（只带入不自动跑检索）；④ 菜单 `separator()` 分组。单测 103→104 | Agent |
| 2026-09-07 | **修复「内容区文字特别小」**：用户反馈正文字号变得极小、看不清。根因两层——① 直接原因：`prefs.txt` 中 `font_size=8.0`（已被长按 ⌘- 缩到底，8.0 恰为原 `MIN_FONT_SIZE` 下限）；② 潜在 bug：字号缩放快捷键用 `consume_shortcut`，而 egui 0.36 的 `consume_shortcut → consume_key → count_and_consume_key` 对 **key-repeat 事件同样返回 true**（其 `matches!` 用 `pressed: true, ..` 吞掉了 `Event::Key.repeat` 字段），长按 ⌘- 一秒即从 12.5 一路跌到下限并落盘。修复：① `handle_shortcuts` 新增 `zoom_pressed` 闭包——只消费 `Event::Key { pressed: true, repeat: false, .. }` 的非重复按下事件，长按仅缩放一次、需多次短按才能持续缩放（⌘+/⌘-/⌘0 三处缩放改用 `zoom_pressed`）；② `core::prefs::MIN_FONT_SIZE` 由 8.0 提到 9.0，保证最小字号仍清晰可读；③ 新增单测 `min_font_size_is_still_readable` 锁定下限 ≥9.0。单测 104→105，门禁 fmt/clippy(-D warnings)/build 全绿 | Agent |
| 2026-09-07 | **发布 0.0.10**：版本号 `Cargo.toml`/`Cargo.lock`/`docs/prd.md` 0.0.9→0.0.10。本版本相对 0.0.9 的变更：**修复「内容区文字特别小」**——① 字号缩放快捷键（⌘+/⌘-/⌘0）改用忽略 key-repeat 的 `zoom_pressed`（只消费 `Event::Key { repeat: false }` 的非重复按下事件），修复长按 ⌘- 瞬间把字号从 12.5 跌到下限 8.0 并落盘的问题（根因：`consume_shortcut` 对 key-repeat 事件同样返回 true）；② `MIN_FONT_SIZE` 由 8.0 提到 9.0，保证最小字号仍清晰可读。单测 104→105 | Agent |
| 2026-09-07 | **字体优化（多字重目录 → 精选嵌入）**：`assets/fonts` 目录更新为 NotoSerifSC（中文衬线，10 字重）与 SauceCodePro Nerd Font（等宽+Nerd 图标，10 字重）两组，原 `MiSans-Normal.ttf` 被移除导致编译失败。修复并优化：① **中文兜底**——`setup_fonts` 用 `NotoSerifSC-Regular.ttf` 替换 MiSans，仍追加到 Proportional/Monospace 两族末尾做 CJK 兜底（衬线体观感，用户确认保持全量、不做子集化）；② **Monospace 主字体**——`SauceCodeProNerdFont-Regular.ttf` 经 `families[Monospace].insert(0,..)` 前置替换内置 Hack，拉丁/代码/符号走它，顺带覆盖 Nerd Font 图标字形；③ **字宽常量校准**——SauceCodePro 拉丁 advance=0.6em、NotoSerifSC 中文=1.0em（fonttools 实测），`estimate_text_width` 的 `ASCII_W` 7.2→7.5、`WIDE_W` 12.2→12.5，保证横向滚动范围估算准确；④ **体积治理**——`.gitignore` 只保留 2 个 Regular 字重入库、其余 18 个变体（141MB→16.4MB）本地保留不提交。release 二进制 19M→28M。门禁 fmt/clippy(-D warnings)/105 单测全绿，headless 探活无 panic | Agent |
| 2026-09-07 | **发布 0.0.11**：版本号 `Cargo.toml`/`Cargo.lock`/`docs/prd.md` 0.0.10→0.0.11。本版本相对 0.0.10 的变更（字体优化）：① **Monospace 主字体**——`SauceCodeProNerdFont-Regular.ttf` 前置替换内置 Hack，日志正文代码/符号显示更佳 + Nerd Font 图标字形覆盖；② **中文兜底**——`NotoSerifSC-Regular.ttf` 替换 MiSans（衬线体，保持全量不做子集化）；③ **字宽常量校准**——`ASCII_W` 7.2→7.5、`WIDE_W` 12.2→12.5（随新字体 advance 实测校准）。release 二进制 19M→28M | Agent |
| 2026-09-07 | **字号定稿 18.0 + 缩小行间距**：用户最终确认日志正文默认字号要 18.0，并要「行与行间距缩小」。① `core::prefs::DEFAULT_FONT_SIZE` 14.0→18.0（正文明显放大）；② `MIN_FONT_SIZE` 11.0→12.0（随默认字号抬，保证最小字号仍清晰）；③ `ui/log_view.rs::ROW_HEIGHT` 18.0→15.0——行高由 `font_size × (ROW_HEIGHT/LOG_FONT_SIZE)` 派生，`ROW_HEIGHT/LOG_FONT_SIZE` 即行距倍率，18/12.5≈1.44 → 15/12.5=1.2，字号放大同时行距更紧凑；④ 同步修 `reset_font_size_returns_to_default`（`set_font_size(18.0)` 撞新默认值改 24.0）与 `min_font_size_is_still_readable`（断言 `>=11.0`→`>=12.0`）。门禁 fmt/clippy(-D warnings)/105 单测全绿 | Agent |
| 2026-09-07 | **内容区字体/字号/配色深度优化**：审查发现四处问题并修复。① **中文字重**——`FONT_CJK` 由 `NotoSerifSC-Regular`(400) 改 `NotoSerifSC-Black`(900)：衬线体 Regular 笔画纤细、同字号下视觉明显偏小（用户反复反馈「字小」），Black 字重笔画粗壮更醒目。注：**"Black" 是字重而非"黑体"（无衬线）**，仍是衬线宋体，解决「笔画细」但不解决「衬线 vs 等宽代码体」的风格差异；各字重 CJK advance 均为 1.0em，故 `WIDE_W=12.5` 无需重校。② **内边距随字号缩放**——`GUTTER_PAD`/`TEXT_PAD` 原固定 8px，字号放大到 18 后留白相对变窄发挤；新增 `gutter_pad_for`/`text_pad_for` 按 `font_size/LOG_FONT_SIZE` 等比缩放（`paint_row_bg` 内由已传入的 `gutter_size` 推导，避免第 8 个参数触发 clippy `too_many_arguments`）。③ **选中行增强**——`row_active` 暗色 0x33383D→0x3D4A5C、亮色 0xDADEE4→0xC2D3E8，与 `row_hover` 拉开可辨差距（原仅差一档灰，选中后肉眼难辨）；光标行指示条 2px→3px（`CURSOR_BAR_W`）。④ **结果页字号脱节**——`results_view` 原硬编码 `LOG_FONT_SIZE`(12.5)，与放大后的正文 18 割裂，改跟随 `state.font_size`，行高倍率同步改 1.2 与正文一致。门禁 fmt/clippy(-D warnings)/105 单测全绿（含 `log_text_colors_stay_readable` 对比度守卫），release 探活无 panic | Agent |
| 2026-09-07 | **正文观感回调 + 多种字体 + 拖选即搜**：用户反馈 18.0 + `Black`(900) 组合「变大、变粗」，三项调整。① **字号回落**——`core::prefs::DEFAULT_FONT_SIZE` 18.0→16.0（`MIN_FONT_SIZE` 12.0 不动）。② **多种字体**——`setup_fonts` 由「2 个字体」扩为「3 个字体 / 3 个族」：CJK 兜底 `NotoSerifSC-Black`(900)→**`NotoSerifSC-Medium`(500)**（核心约束是**字重与拉丁主字体相称**：拉丁固定 400，中文 900 造成「英细中粗」割裂，400 又因衬线笔画纤细显小，500 兼顾）；新增 `SauceCodeProNerdFont-Bold` 注册为独立族 `theme::FONT_FAMILY_MONO_BOLD`（**不并入 Monospace，否则全文变粗**），仅 `build_job` 的命中段用它做「底色 + 字重」双重强调；该族必须追加 CJK 兜底，否则命中的中文豆腐块。`.gitignore` 同步只入库 Medium/Regular/Bold 三个字重，并把已入库但不再使用的 Black/Regular 用 `git rm --cached` 移出索引（本地文件保留）。③ **拖选即搜**——egui 不暴露 `Label` 选区文本（`LabelSelectionState` 只有 `has_selection`，拿不到字符串），故在 `log_view` 自行实现正文拖选：每帧取一次指针快照（`press_origin`/`interact_pos`/`primary_down`），按下行内记起点字符索引，拖动中用 `char_index_at_offset`（复用双击取词的「排版一次→`cursor_from_pos`」路径）反查当前索引，`selected_text_between` 取出文本写入 `PaneState::selected_word`，`⌘F` 原有链路即自动带入检索框。抽 `char_index_at_offset` 供取词与拖选共用；新增 `PaneState::drag_text`；拖动过文字时抑制随后的「点击选中整行」。跨行拖选不支持（只取起点行内部分）。单测 105→108（新增 `selected_text_between` 三组） | Agent |
| 2026-09-07 | **发布 0.0.12**：版本号 `Cargo.toml`/`Cargo.lock`/`docs/prd.md` 0.0.11→0.0.12。本版本相对 0.0.11 的变更（内容区观感三连）：① **字号定稿 18.0 + 行距缩小**——`DEFAULT_FONT_SIZE` 14→18（后回调至 16，见下条）、`ROW_HEIGHT` 18→15（行距倍率 1.44→1.2）；② **内容区深度优化**——CJK 字重 Black(900)→Medium(500) 与拉丁 400 相称、`GUTTER_PAD`/`TEXT_PAD` 随字号等比缩放、`row_active` 与 `row_hover` 拉开可辨差距（暗 0x33383D→0x3D4A5C / 亮 0xDADEE4→0xC2D3E8）、光标指示条 2→3px、结果页字号跟随 `state.font_size`（原硬编码 12.5 与正文割裂）；③ **正文观感回调 + 多种字体 + 拖选即搜**——`DEFAULT_FONT_SIZE` 18→16（18+Black 组合「变大变粗」）、字体扩为 3 个（新增 `SauceCodeProNerdFont-Bold` 独立族 `MonoBold` 专供命中加粗）、正文拖选写入 `selected_word` 供 `⌘F` 带入。单测 105→108，release 二进制 31M，门禁 fmt/clippy(-D warnings)/探活全绿 | Agent |
