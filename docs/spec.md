...
#### 11.2.1 实测值（M6，`--release`，测试机 Apple M 系列，样本 `gen_log.sh /tmp/bench_1gb.log 10000000`）

| # | 指标 | 目标 | 实测 | 结论 |
| --- | --- | --- | --- | --- |
| P1 | 打开 1 GB（≈1000 万行） | < 2.0 s | **0.982 s** | ✅ 余量充足（并行 `memchr` 扫描替代单线程逐字节循环） |
| P3 | 索引常驻内存 | ≤ 8 B/行 | 8.00 B/行（80 MB / 1000 万行） | ✅ 恰好达标（`Vec<usize>` 8 字节/行） |
| P7 | 正则检索吞吐 | ≥ 200 MB/s | **578.7 MB/s** | ✅ |
| P8 | 纯文本（字面量）检索吞吐 | ≥ 800 MB/s | **803.7 MB/s** | ✅ 余量偏小（大小写不敏感字面量走 NFA，非纯 `memchr`） |

> **说明**：M6 前首次测量 P1=3.94 s、P8=404 MB/s 均未达标，根因为单线程逐字节扫描与逐行
> `from_utf8_lossy` 分配；经 M6 优化（`build_line_offsets` 并行 `memchr`、检索改 `regex::bytes`
> 整块 `find_iter` + 偏移二分映射行号）全部达标。
> 未自动化项（需 GUI 手工观测，代码已就位）：P2/P4/P5/P6/P9/P10/P11/P12。
> 测量命令：`cargo test --release -- --ignored open_1gb_under_2s index_memory_within_budget search_throughput_gb`
